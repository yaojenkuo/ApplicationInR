...
stateShp <- readShapePoly('ne_10m_admin_1_states_provinces.shp')
...
stateShpSimplify <- gSimplify(stateShpSubsetCorrection,tol=0.005, topologyPreserve=TRUE)
...
stateShpDataOrder <- stateShpDataCorrection[order(stateShpDataCorrection$name),]#Sort
ISAttacks2015Order <- ISAttacks2015[order(ISAttacks2015$state),]#Sort
rownames(ISAttacks2015Order) <- rownames(stateShpDataOrder)#Very important!
stateShpDataOrder <- cbind(stateShpDataOrder, ISAttacks2015Order[,c(1,2,4,5)])#Merge
colnames(stateShpDataOrder)[1] <- c('state')#Change column name
...
# Create GeoJSON
writeOGR(ISAttackSPDF, 'C:/Users/ASUS/Documents/ApplicationInR/ISAttacks2015/ISAttacks2015.js', layer="", driver="GeoJSON")